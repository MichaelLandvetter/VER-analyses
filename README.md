...
- Use **Suggest Exclusion** (next to **Open Data File**) to open the **Exclusion Tuning** dialog, which shows a whole-file downsampled filtered signal time-series plot with draggable symmetric ±T threshold lines. Drag either line vertically to set the exclusion threshold directly on the signal — matching the clinical workflow of visually inspecting the trace. A slider and spinbox provide fine-grained numeric control and remain synchronized with the draggable markers. Live accepted/rejected epoch counts and the auto-suggested and current thresholds are shown as reference.
